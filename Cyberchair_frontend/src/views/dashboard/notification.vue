            ...
            <v-container>
              ...
              >
                <v-card
                  color="white"
                >
                  <v-card-text style="color: black" v-text="message['info']">
                  </v-card-text>

                </v-card>

              </v-col>
              <div v-if="messages.length===0">No message</div>
            </v-container>
          </v-app>
...
export default {
  ...
  methods: {
    ...
    sendfeedback() {
      ...
      this.$axios.post('api/pcmember-service/user/invitationRepo',
        {
  ...
  mounted: function () {
    var that = this;
    this.$axios.get('api/pcmember-service/user/alreadyDealedNotifications',
    ...
    this.$axios.get('/api/pcmember-service/user/undealedNotifications',
      {params: {username: localStorage.username}}
    ).then(resp => {
      if (resp.data.responseCode == 200 && resp.data.responseMessage == "success") {
    ...
    //pc member 邀请结果
    this.$axios.get('/api/pcmember-service/getInvitationResultMessagesByChairName',
      {params: {chairName: localStorage.username}}
    ).then(resp => {
      if (resp.data.responseCode == 200 && resp.data.responseMessage == "success") {
        var result = resp.data.responseBody.messages;
    ...
    //会议收到投稿通知
    this.$axios.get('/api/article-service/getArticleMessagesByChairName',
      {params: {chairName: localStorage.username}}
    ).then(resp => {
      if (resp.data.responseCode == 200 && resp.data.responseMessage == "success") {
        var result = resp.data.responseBody.messages;
    ...
    //会议开启审稿或复审通知
    this.$axios.get('/api/pcmember-service/getReviewingMeetingMessagesByPCMemberName',
      {params: {pcMemberName: localStorage.username}}
    ).then(resp => {
      if (resp.data.responseCode == 200 && resp.data.responseMessage == "success") {
        var result = resp.data.responseBody.messages;
        for (var i = 0; i < result.length; i++) {
    ...
    //论文录用结果通知
    this.$axios.get('/api/article-service/getArticleResultMessagesByAuthorName',
      {params: {authorName: localStorage.username}}
    ).then(resp => {
      if (resp.data.responseCode == 200 && resp.data.responseMessage == "success") {
        var result = resp.data.responseBody.messages;
        for (var i = 0; i < result.length; i++) {
